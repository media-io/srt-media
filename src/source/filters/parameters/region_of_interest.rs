...
impl RegionOfInterest {
  pub fn get_video_crop(&self, image_width: u32, image_height: u32) -> Result<VideoCrop, String> {
    match self.clone() {
      RegionOfInterest {
        top: Some(top),
        left: Some(left),
...
pub fn region_of_interest_to_coordinates_top_left_right_bottom() {
  ...
  let coordinates = region_of_interest.get_video_crop(600, 400).unwrap();
...
pub fn region_of_interest_to_coordinates_errors() {
  ...
  let error = roi.get_video_crop(600, 400).unwrap_err();
  let expected = format!(
    "Cannot compute coordinates from such a region of interest: {:?}",
    roi
  );
